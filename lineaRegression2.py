...
model_file_name = "modelLinearRegressionSadHappy.dat"
momentum_factor = 0.9
...
for i in range(8):
    plt.subplot(331 + i)
    plt.imshow(model['weight'][i].reshape(64,64), cmap=plt.cm.gray)

fig.suptitle('Weights for class before training')
plt.show()
...
def train(model, data):
    ...
    # start form 0
    w_mom_speed = np.zeros(w.shape)
    b_mom_speed = np.zeros(b.shape)
    ...
    for iter in range(num_iter):
        ...
        #    loss = -(Y*np.log(y_hat) + ((1-Y)*np.log(1 - y_hat)))
            ...
            # update parameters
            w_mom_speed = momentum_factor * w_mom_speed + dw
            b_mom_speed = momentum_factor * b_mom_speed + db

            w = w - (learning_rate*w_mom_speed)
            b = b - (learning_rate*b_mom_speed)
...
for i in range(8):
    plt.subplot(331+i)
    plt.imshow(model['weight'][i].reshape(64,64), cmap=plt.cm.gray)
fig.suptitle('Weights for class after training')
plt.show()
...
if True:
    ...
    for i in range(num_tests):
        ...
        if emotion[row] != emo:
            ...
            fname, flip_id = dataTest['fname'][i]
